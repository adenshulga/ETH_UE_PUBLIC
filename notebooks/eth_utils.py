...
def plot_forecast(model, test_dataset, idx):
    ...
    plt.fill_between(
        ...
        upper_bound[:, 1],
        alpha=0.5,
        label=f"forecast Low confidence {alpha:.2f}",
    )
    plt.legend()
...
def plot_quantile_forecast(model, test_dataset, columns, idx):
    output_range = range(model.input_len, model.input_len + model.output_len)
    full_range = range(0, model.input_len + model.output_len)
    ...
    for c, color in enumerate(["tab:blue", "tab:orange"]):
        ...
        plt.title(f"Example of forecast {[columns[c]]}")
        plt.grid()
    plt.show()


def print_crossing_ratio(model, test_dataset, idx=0):
    input_seq, output_seq = test_dataset[idx]
    output_seq = output_seq[None, -model.output_len :]
    input_seq = input_seq[None, :, :]
    predicted_seq = model.predict(input_seq)
    ratio = (
        (predicted_seq.diff(dim=3) >= 0).sum()
        / model.output_len
        / model.target_dim
        / (len(model.quantile_levels) - 1)
    )
    print(f"Quantile crossing ratio: {1 - ratio:.4f}")


def plot_quantile_levels(model, test_dataset, idx=0):
    input_seq, output_seq = test_dataset[idx]
    output_seq = output_seq[None, -model.output_len :]
    input_seq = input_seq[None, :, :]
    predicted_seq = model.predict(input_seq)
    plt.plot(model.quantile_levels, predicted_seq[0, 0, 0, :])
    plt.xlabel("Quantile level")
    plt.ylabel("Quantile value")
    plt.title("Example of forecast of quantiles")
    plt.grid()
    plt.show()
